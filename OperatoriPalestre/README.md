...
OperatoriPalestre è una webapp che, successivamente ad una fase di login che avviene da parte di un utente operatore, permette tramite comunicazione con un’applicazione server di gestire le disponibilità dei facilitatori riguardanti le palestre digitali.
...
Le webapp in questione è raggiungibile al seguente link: [https://officinedigitali.fcc.veneto.it/OperatoriPalestre/](https://officinedigitali.fcc.veneto.it/OperatoriPalestre/)

Per utilizzarla, gli operatori ai quali sono state fornite delle credenziali dovranno superare una fase di login. Fatto questo, potranno visualizzare le ore fisse e le disponibilità richieste per il mese corrente riguardanti il facilitatore associato alla palestra a cui loro stessi sono associati e, durante la seconda metà del mese, richiedere al facilitatore le disponibilità per il mese successivo.
...
È necessario copiare nel proprio dispositivo i file od_gyms.sql e le cartelle OperatoriPalestre e Apps_server, presenti nella repository raggiungibile attraverso l’URL [https://github.com/FCC-OfficineDigitali/app-mappe-e-raccolta](https://github.com/FCC-OfficineDigitali/app-mappe-e-raccolta).

Per poter usufruire in locale di una copia di OperatoriPalestre, è necessario mantenere attivo MySQL tramite XAMPP per utilizzare il database MySQL il cui file .sql è contenuto nella repository.
...
Se non si conoscono le credenziali di alcun operatore, è necessario modificare dal database la password di uno di loro mettendo, nella colonna password delle rispettive entry presenti nelle tabelle operators e accounts, il risultato della funzione [SHA-1](http://www.sha1-online.com/) applicata alla password desiderata.
