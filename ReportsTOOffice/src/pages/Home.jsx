...
export default function Home() {
    ...
    useEffect(_ => {
        if (category !== null) {
            Axios.get(ipPortApi + "getReports/" + (category * -1) + "O&tkn=" + token)
                .then(data => {
        ...
        <>
            <Box style={{ minHeight: "101vh", display: "flex", flexDirection: "column" }}>
                ...
                <Box style={{ padding: "24px 8%" }}>
                    <Box my={sm ? 4 : 1}>
                        <Tabs value={category} onChange={(event, value) => setCategory(value)} variant="fullWidth" sx={{ borderRadius: "4px" }} TabIndicatorProps={{ sx: { backgroundColor: category === 0 ? theme.palette.primary.selectedGreen : theme.palette.primary.selectedOrange } }}>
                            {[1, 0].map((element, index) =>
                                <StyledTab label={"Segnalazioni " + (element === 1 ? "aperte" : "chiuse")} value={element} key={index}
                                    sx={{
                    ...
                    {
                        category === 0 && rowsListFull.length > 0 &&
                        <Box
                            sx={{
                                display: "grid",
                                gridTemplateColumns:
                                ...
                                <Accordion p={1} key={category + "_" + pageNr + "_" + index + "_" + val.report_id} disableGutters={true} sx={{ border: "1px solid" + theme.palette.primary.main + "22" }}>
                                    ...
                                    >
                                        <Box>
                                            ...
                                            {val.phone &&
                                                <Typography variant="data"> dall'utente contattabile al numero{" "}
                                                    <a title={"Chiama " + val.phone} href={"tel:+39" + val.phone} style={{ color: theme.palette.primary.main }}>{val.phone}</a>
                                                </Typography>
                                            }
                                            <Typography variant="data">
                                                        ...
                                                        </Box>
                                                        {
                                                            ...
                                                            <>
                                                                <Box
                                                                    ...
                                                                    <ReportsDialogBtnComp updateListFun={handleClickToUpdate} report={val} />
                                                                </Box>
                                                            </>
                                                        }
                                                    </>
