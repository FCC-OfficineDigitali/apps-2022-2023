...
Segnalazioni Treviso-Ostiglia è una webapp che, successivamente ad una fase di login che avviene da parte di un segnalatore, permette tramite comunicazione con un’applicazione server di aprire e inoltrare, ad un apposito ufficio, segnalazioni riguardanti problemi rilevati lungo la Treviso-Ostiglia.
...
Le webapp in questione è raggiungibile al seguente link: [https://officinedigitali.fcc.veneto.it/ReportsTOSignallers/](https://officinedigitali.fcc.veneto.it/ReportsTOSignallers/)

Per utilizzarla, i segnalatori dovranno superare una fase di login. Fatto questo, potranno visualizzare le proprie segnalazioni aperte e inserirne di nuove, oltre che visualizzare e filtrare le segnalazioni chiuse.

È inoltre possibile modificare le proprie credenziali.
...
È necessario copiare nel proprio dispositivo i file od_reports.sql e le cartelle ReportsTOSignallers e Apps_server, presenti nella repository raggiungibile attraverso l’URL [https://github.com/FCC-OfficineDigitali/app-mappe-e-raccolta](https://github.com/FCC-OfficineDigitali/app-mappe-e-raccolta).

Per poter usufruire in locale di una copia di Segnalazioni Treviso-Ostiglia, è necessario mantenere attivo MySQL tramite XAMPP per utilizzare il database MySQL il cui file .sql è contenuto nella repository.
...
Se non si conoscono le credenziali di alcun segnalatore, è possibile crearne uno tramite l’apposito form selezionando “registrati” sulla pagina di login.
